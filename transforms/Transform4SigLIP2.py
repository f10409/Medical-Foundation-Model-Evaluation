...
class SigLIPProcessor(MapTransform):
    """
    MONAI transform for processing medical images using Google's SigLIP2 processor.
    
    Converts medical images to 8-bit RGB format and applies SigLIP2 preprocessing.

    Args:
        keys (KeysCollection): Keys of the images to be transformed
        processor_name (str): HuggingFace model repository name (default: "google/siglip2-so400m-patch16-512")
        max_length (int, optional): Maximum sequence length for processor compatibility (default: None)
    """
    
    def __init__(self, keys, processor_name="google/siglip2-so400m-patch16-512", max_length=None):
        super().__init__(keys)
...
class Transform4SigLIP2:
    """
    MONAI transform composition for processing medical images with SigLIP2.
