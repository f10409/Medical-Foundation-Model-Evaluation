...
def get_data_dict_part(df_part, config):
    ...
    for i in tqdm(range(len(df_part)), desc="Processing part"):
        ...
        data_dict.append({
            ...
            ** {l: np.array([row[f'{l}']]) for l in LABELS}
        })
...
def get_dataloader(input_df, config):
    ...
    #     train_transforms = Transform4MedSigLIP(CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4MedSigLIP(CLASSES=config.labels).cla_transforms
    # if config.model_name == "microsoft/rad-dino":
    #     train_transforms = Transform4RADDINO(IMG_SIZE=518, CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4RADDINO(IMG_SIZE=518, CLASSES=config.labels).cla_transforms
    # if config.model_name == "StanfordAIMI/XraySigLIP__vit-l-16-siglip-384__webli":    
    #     train_transforms = Transform4CheXagent(IMG_SIZE=512, CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4CheXagent(IMG_SIZE=512, CLASSES=config.labels).cla_transforms
    # if config.model_name == "hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224": 
    #     train_transforms = Transform4BiomedCLIP(IMG_SIZE=224, CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4BiomedCLIP(IMG_SIZE=224, CLASSES=config.labels).cla_transforms
    # if config.model_name == "google/siglip2-so400m-patch16-512":
    #     train_transforms = Transform4SigLIP2(CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4SigLIP2(CLASSES=config.labels).cla_transforms
    # if config.model_name == "facebook/dinov2-base":
    #     train_transforms = Transform4DINOv2(CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4DINOv2(CLASSES=config.labels).cla_transforms
    # if config.model_name == "MedImageInsights":
    #     train_transforms = Transform4MedImageInsights(CLASSES=config.labels).cla_transforms
    #     val_transforms = Transform4MedImageInsights(CLASSES=config.labels).cla_transforms
    if config.model_name == "Ark+":
        train_transforms = Transform4Ark(CLASSES=config.labels).cla_transforms
        val_transforms = Transform4Ark(CLASSES=config.labels).cla_transforms
