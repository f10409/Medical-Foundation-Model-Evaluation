...
"""
FM Segmentation Evaluation - Function Version

This module evaluates the performance of fine-tuned FM segmentation models on medical image segmentation tasks.
"""
...
def evaluate_segmentation_model(config=None, weight_path_override=None, integrated=False):
    """
    Evaluate a fine-tuned FM segmentation model on test data
    ...
    print("=== FM Segmentation Model Evaluation ===")
    ...
    _, valid_dataloader, test_dataloader = get_seg_dataloader(input_df, config)
    print(f"Validation batches: {len(valid_dataloader)}")
    print(f"Test batches: {len(test_dataloader)}")

    # Initialize FM segmentation model
    print("\n=== Initializing Model ===")
    if integrated:
    ...
    if weight_path_override and weight_path_override.endswith('.ckpt'):
        # If a specific checkpoint file is provided
        checkpoint_path = weight_path_override
        print(f"Using specific checkpoint: {checkpoint_path}")
    else:
