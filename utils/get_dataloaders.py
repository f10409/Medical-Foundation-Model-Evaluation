...
from transforms.Transform4SegFormer import Transform4SegFormer
import numpy as np
...
def get_dataloader(input_df, config):
    ...
    if config.model_name == "nvidia/segformer-b4-finetuned-ade-512-512":
        train_transforms = Transform4SegFormer(CLASSES=config.labels).cla_transforms
        val_transforms = Transform4SegFormer(CLASSES=config.labels).cla_transforms
